...
import requests
import collections
from src.aqjr import Des, sm4decode
    ...
    def excutive_test(cls, profix_url, interface_type, content):
        if interface_type in ["login", "sell", "encash"]:
            ...
            if interface_type == 'time' and "RespContent" in recv_post:
                ...
                server_time = recv_post["RespContent"]['SysDateTime']
                time = "%s-%s-%s %s:%s:%s" % (server_time[0:4], server_time[4:6], server_time[6:8],
                                              server_time[8:10], server_time[10:12], server_time[12:])
                basic_data.logger.info('time接口返回成功，time: %s' % time)
                return time
                ...
                if tag == 'encash':
                    with open(basic_data.result_dir + "encash_detail_record.txt", 'w+') as encash_detail_file:
                        for record in f:
                            ...
                            checkcode = Des.getcheckcode(content.encash_content['UserId'] +
